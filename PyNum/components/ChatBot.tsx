...
export const ChatBot: React.FC<ChatBotProps> = ({ currentAlgo, isOpen, onClose }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Pass the whole history to maintain conversational state
      const answer = await askAiTutor(updatedMessages, currentAlgo);
      setMessages(prev => [...prev, { role: 'model', text: answer }]);
    } catch (err) {
      setMessages(prev => [...prev, { role: 'model', text: "I'm having trouble connecting to my knowledge base. Please try again later.", isError: true }]);
    } finally {
      setIsLoading(false);
    }
  };
