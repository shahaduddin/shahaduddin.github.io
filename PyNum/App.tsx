...
import { HashRouter, Routes, Route, Navigate, useParams, useNavigate, useLocation, Link } from 'react-router-dom';
...
// Helper to slugify algorithm names for URLs
...
const MainLayout: React.FC = () => {
  ...
  // Extract slugs from the URL path directly since useParams won't work outside Route
  ...
  // Production optimization: Update page title based on current method
  useEffect(() => {
    document.title = `${currentAlgo} - PyNumAnalysis Studio`;
  }, [currentAlgo]);
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative">
        <header className="h-14 px-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between z-30 shrink-0 transition-colors shadow-sm">
          ...
          <div className="flex items-center gap-3">
             <div className="hidden sm:flex items-center gap-1 text-[9px] font-black text-emerald-600 dark:text-emerald-400 uppercase bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded border border-emerald-100 dark:border-emerald-800">
             ...
             <div className="relative" ref={settingsRef}>
               ...
               {isSettingsOpen && (
                 <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 py-2 animate-in zoom-in-95 origin-top-right overflow-hidden transition-colors">
        ...
        <Routes>
          <Route path="/" element={<Navigate to={`/${slugify(TopicCategory.ROOTS)}/${slugify(AlgorithmType.BISECTION)}/demo`} replace />} />
