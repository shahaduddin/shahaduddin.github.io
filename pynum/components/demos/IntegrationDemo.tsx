...
export const IntegrationDemo: React.FC<IntegrationDemoProps> = ({ type }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {result !== null && (
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 text-center shadow-xl border border-slate-200 dark:border-slate-800 relative overflow-hidden group transition-colors">
          <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <Calculator className="w-32 h-32 text-emerald-600" />
          </div>
          <span className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] block mb-2 relative z-10">Approximate Definite Integral</span>
          <span className="text-5xl md:text-7xl font-bold text-slate-900 dark:text-emerald-400 font-mono tracking-tighter relative z-10">{result.toFixed(8)}</span>
        </div>
      )}
